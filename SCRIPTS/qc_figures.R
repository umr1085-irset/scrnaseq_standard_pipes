#####################################
# Basic QC figures for prepipe & pipe
# 2021 IRSET
# Fred CHALMEL
#####################################
...
#sel.genes3 <- sort(rownames(seurat.obj)[ toupper(rownames(seurat.obj)) %in% c("AKAP12", "RSPO1", "NR2F2", "INHBA", "CYP26B1", "SCUBE1", "DLX5", "MAFB", "GLI1", "MFAP5", "FOXL2", "SOX9")])
sel.genes  <- unique(c(sel.genes1,sel.genes2, sel.genes3))
sel.genes  <- sort(sel.genes[ which( is.na(sel.genes) == FALSE ) ])
#--------------------------------------#
